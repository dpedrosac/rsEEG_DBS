...
# General variables for the entire script
FLAG_CHECK = False
SPIKE_ROWS = 8
SCALE_FACTOR = 2
...
sample_data_raw_file = Path.home() / 'Schreibtisch' / 'rsEEG' / 'rsEEG_15' / 'DBS_15_1.vhdr'
...
if FLAG_CHECK:
    fig = raw.plot(proj=False, n_channels=2)

raw_highpass = raw.copy().filter(l_freq=1, h_freq=90)

# raw_processed = RawArray(data=data_restored, info=raw_highpass.info)
# filtered_psd=raw_processed.compute_psd(fmin=0, fmax=250, n_fft=2048, picks='eeg').plot()
...
def spike_detection(spikes,
    ...
    if type == 2 and FdbsL is not None and FdbsR is not None:
        ...
        for spk in SpikesIndex:
            ...
            dbs_induced, n, h = test_spike(Fs, FdbsL, FdbsR, nmax, eps, 0)
...
def prepare_spike_detection(raw_data: object,
    ...
    Y, f = compute_fft(eeg_data_input, sampling_rate)
...
def spike_removal(raw_eeg_data : object, sampling_rate: float, spikes):
    """
    ...
    # Compute FFT using helper function
    Y, f = compute_fft(raw_eeg_data, sampling_rate)
    logging.info(f"FFT result shape: {Y.shape}, Frequency vector length: {len(f)}.")
